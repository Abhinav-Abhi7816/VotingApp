...
route.post('/signUp', async (req, res) => {
    try {
        ...
        if (await Voter.findOne({ username: data.username })) {
            return res.status(400).json({ message: 'username is already in use! Try another!' })
        }
        if (await Voter.findOne({ aadharNo: data.aadharNo })) {
            return res.status(400).json({ message: 'Aadhar Number already used! Try another!' })
        }
        if (await Voter.findOne({ voterId: data.voterId })) {
            return res.status(400).json({ message: 'Voter Id already used! Try another!' })
        }
        if (await Voter.findOne({ email: data.email })) {
            return res.status(400).json({ message: 'email is already in use! Try another!' })
        }
        if (await Voter.findOne({ mobileNo: data.mobileNo })) {
            return res.status(400).json({ message: 'Mobile number already used! Try another!' })
        }
